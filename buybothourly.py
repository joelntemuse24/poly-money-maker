...
from buy.depth_ladder import (
    ...
    DepthPathBuffer,
    TOPUP_ASSUMPTION,
    append_depth_ladder_jsonl,
    available_at_limit,
    compute_depth_ladder,
    make_depth_sample,
    simulate_topup_path,
    summarize_ask_levels,
)
...
DEPTH_LADDER_JSONL = "buy_data_hourly/depth_ladder.jsonl"
DEPTH_LADDER_BUDGETS = (5.0, 10.0, 20.0, 32.0, 40.0, 50.0, 100.0)
DEPTH_TOPUP_JSONL = "buy_data_hourly/depth_topup.jsonl"
...
_STRATEGY_DEFAULTS = {
    ...
    "early_hot_defer_edge_usd": 200.0,
    # While early-hot is active, a22 may spend only this fraction of a22_buy_budget;
    # remaining is available as a top-up once early-hot clears. b15 is never
    ...
    # Cap early-rich asks strictly below early_rich_take_profit_bid so a
    # 99¢ buy cannot TP/sell at 99¢ (zero/negative lock). Default TP−0.01.
    "early_rich_a22_ask_max": 0.98,
    "early_rich_a22_persist_s": 90.0,
    "early_rich_a22_window_min": 20.0,
    "early_rich_a22_min_edge_usd": 60.0,
    # Early-rich bags: full exit at 99¢ (not global take_profit_full_bid).
    # Never post sell limit below this bid; wait if book is soft under TP.
    ...
    "entry_persist_ttm_frac": 0.5,
    "entry_persist_ttm_floor_s": 5.0,
    ...
    "hedge_max_spread": 0.20,
    "hedge_require_ask_max": 0.52,
    "hedge_persist_s": 5.0,
    # 0 disables toxic/instant dump; normal hedge_threshold scratch still works.
    "hedge_toxic_bid_max": 0.0,
    # Toxic dump must stay <= dump this many seconds (0 = instant).
    "hedge_dump_persist_s": 0.0,
    # Mid window: TTM <= hedge_dump_mid_ttm_s → hedge_dump_mid_persist_s
    # (last 5 minutes → 6s).
    "hedge_dump_mid_ttm_s": 300.0,
    "hedge_dump_mid_persist_s": 6.0,
    # Late window: TTM <= hedge_dump_late_ttm_s → hedge_dump_late_persist_s
    # (last 1 minute → 2s; was 15s/2s after 10pm cliff).
    "hedge_dump_late_ttm_s": 60.0,
    "hedge_dump_late_persist_s": 2.0,
    # If favor-edge peaked >= peak_usd then bled to <= now_usd while still
    # favoring the held leg, allow hedge/dump (do not block on
    # oracle_still_winning_small).
    "hedge_edge_collapse_peak_usd": 40.0,
    "hedge_edge_collapse_now_usd": 10.0,
    # Hourly: dump needs ask + spread <= hedge_max_spread (blocks 34/99).
    "hedge_dump_require_tight": True,
    "hedge_dump_ignore_spread_ask_max": 0.60,
    # Dump freshness: WS age ≤ this OR successful REST; last_dump alone
    # may skip REST only when last_good age ≤ this (blocks 64–91s stale).
    "hedge_dump_max_quote_age_s": 2.0,
    "hedge_dump_require_fresh_book": True,
    # Refuse toxic DUMP while oracle still favors held by more than this
    # (0.0 = any positive favor_edge blocks dump).
    "hedge_dump_max_favor_edge_usd": 0.0,
    # Edge-collapse unlocks normal hedge/sell only; dump needs this True.
    "hedge_edge_collapse_allows_dump": False,
    # Toxic DUMP exit: seek better FAK fills HIGH→LOW toward live bid.
    # Prefer ≥ floor first; if still unfilled, keep stepping below floor
    # down to max(live_bid, tick). late_sweep_ttm skips straight to live bid.
    ...
    # When TTM below this, skip straight to max(live_bid, tick).
    "hedge_dump_late_sweep_ttm_s": 45.0,
    ...
    "buy_max_shares": 14.0,
    # Size FAK share count as budget/ref so win payout (~shares) stays stable
    # when entry is above the band floor. true = target shares; false = old
    # maximize-spend-under-budget. Refs default from floors when 0:
    # a22 -> a22_min_price, b15 -> buy_threshold, c5 -> c5_min_price.
    # Optional explicit a22_size_ref_price / b15_size_ref_price / c5_size_ref_price
    # (e.g. 0.95 / 0.90) override when > 0.
    "size_to_ref_price": True,
    "a22_size_ref_price": 0.0,
    "b15_size_ref_price": 0.0,
    "c5_size_ref_price": 0.0,
    ...
    "tick_size": "0.01",
# Take-profit: sell when live bid >= blended cost + edge.
# Grandfather via take_profit_from_start_ts so the open 9AM ET bag is
# excluded. 1788789600 = 10:00 America/New_York 2026-09-07 (verified).
# 0 = all markets. Bypass oracle hold (winning bags never flip).
    "take_profit_enabled": False,
    "take_profit_edge": 0.04,
    "take_profit_from_start_ts": 0.0,
    # Bid must stay at/above target this many seconds before sell (0 = instant).
    "take_profit_persist_s": 5.0,
    # Fraction of held size to sell once (0.5 = half bag); remainder rides.
    ...
    # Soft-edge may use a stale-tolerant WS age (hedge stays at 0.25s).
    # Live 12am ET 2026-09-12: arm re-armed ~every 8s and never fired because
    # HEDGE_QUOTE_MAX_AGE_S gaps cleared qualifies before persist_s.
...
def load_strategy():
    ...
    try:
        ...
        # Joel 2026-09-12: allow a22 tighter/later than b15 (e.g. a22 last-10m,
        # b15 last-15m). window_min means enter when TTM <= N, so smaller a22
        # starts later than b15 — intentional.
        ...
        # 0 = toxic dump disabled (hold / normal hedge only).
        ...
        if float(cfg["hedge_toxic_bid_max"]) > 0 and not (
        ...
        if float(cfg["hedge_toxic_bid_max"]) <= 0 and not (
            float(cfg["hedge_threshold"]) <= float(cfg["hedge_recovery_cancel"]) <= 1
        ):
            raise ValueError(
                "hedge_recovery_cancel must satisfy qualify <= recovery_cancel <= 1"
            )
        if not (0 <= float(cfg["take_profit_edge"]) <= 1):
            raise ValueError("take_profit_edge must satisfy 0 <= edge <= 1")
        if float(cfg["take_profit_from_start_ts"]) < 0:
            raise ValueError("take_profit_from_start_ts must be >= 0 (0 = all markets)")
        if float(cfg.get("take_profit_persist_s", 5.0)) < 0:
            raise ValueError("take_profit_persist_s must be >= 0")
        if float(cfg.get("soft_edge_exit_max_usd", 0.0) or 0.0) < 0:
            raise ValueError("soft_edge_exit_max_usd must be >= 0")
        if not (0 <= float(cfg.get("soft_edge_exit_bid", 0.0) or 0.0) <= 1):
            raise ValueError("soft_edge_exit_bid must satisfy 0 <= bid <= 1")
        if float(cfg.get("soft_edge_exit_persist_s", 0.0) or 0.0) < 0:
        ...
        _tp_frac = float(cfg.get("take_profit_fraction", 0.5))
        if not (0 < _tp_frac <= 1):
        ...
        _er_tp_bid = float(cfg.get("early_rich_take_profit_bid", 0.99) or 0.0)
        if _er_tp_bid < 0 or _er_tp_bid > 1:
            raise ValueError("early_rich_take_profit_bid must satisfy 0 <= bid <= 1")
        if float(cfg.get("early_rich_take_profit_persist_s", 5.0) or 0.0) < 0:
        ...
        _er_ask_max = float(cfg.get("early_rich_a22_ask_max", 0.98) or 0.0)
        if not (0 <= _er_ask_max <= 1):
            raise ValueError("early_rich_a22_ask_max must satisfy 0 <= ask <= 1")
        if bool(cfg.get("early_rich_a22_enabled", False)) and float(cfg["a22_window_min"]) > 0:
            ...
            if _er_ask_max - 1e-12 > float(cfg["high_buy_max_price"]):
                raise ValueError(
                    "early_rich_a22_ask_max must be <= high_buy_max_price when early-rich a22 is on"
                )
            if _er_ask_max + 1e-12 < _er_ask:
                raise ValueError(
                    "early_rich_a22_ask_max must be >= early_rich_a22_ask_min when early-rich a22 is on"
                )
            if bool(cfg.get("early_rich_take_profit_enabled", True)):
                _er_tp = float(cfg.get("early_rich_take_profit_bid", 0.99) or 0.0)
                if _er_tp > 1e-12 and _er_ask_max + 1e-12 >= _er_tp:
                    raise ValueError(
                        "early_rich_a22_ask_max must be < early_rich_take_profit_bid "
                        "(leave room under exit)"
                    )
        ...
        if float(cfg.get("early_rich_a22_min_edge_usd", 0.0) or 0.0) < 0:
            raise ValueError("early_rich_a22_min_edge_usd must be >= 0")
        ...
        for key in (
            ...
            "buy_window_min", "ui_every_n_cycles",
        ):
            if float(cfg[key]) <= 0:
                raise ValueError(f"{key} must be positive")
        # b15_window_min may be 0 (disabled), same as a22/c5.
        ...
        if float(cfg["a22_buy_budget"]) > _market_cap + 1e-9:
        ...
        for _ref_key in (
            "a22_size_ref_price", "b15_size_ref_price", "c5_size_ref_price",
        ):
            _ref_v = float(cfg[_ref_key])
            if _ref_v < 0 or _ref_v > 1:
                raise ValueError(f"{_ref_key} must satisfy 0 <= ref <= 1 (0 = derive)")
        if bool(cfg["size_to_ref_price"]):
            _a_ref = float(cfg["a22_size_ref_price"]) or float(cfg["a22_min_price"])
            _b_ref = float(cfg["b15_size_ref_price"]) or float(cfg["buy_threshold"])
            _c_ref = float(cfg["c5_size_ref_price"]) or float(cfg["c5_min_price"])
            for _rk, _rv in (
                ("a22_size_ref", _a_ref),
                ("b15_size_ref", _b_ref),
                ("c5_size_ref", _c_ref),
                ...
                if not (0 < _rv <= 1):
                    raise ValueError(f"{_rk} must satisfy 0 < ref <= 1")
            _ref_need = max(
                float(cfg["a22_buy_budget"]) / _a_ref,
                float(cfg["b15_buy_budget"]) / _b_ref,
                float(cfg["c5_buy_budget"]) / _c_ref,
                _need_shares,
            )
            if float(cfg["buy_max_shares"]) + 1e-9 < _ref_need:
                raise ValueError(
                    "buy_max_shares must be >= max(slice_budget / size_ref) "
                    "when size_to_ref_price is on"
                )
            # Slight overspend at limit > ref (e.g. $5@95 -> 5.26 sh @99¢ ≈ $5.21).
            _worst_spend = max(
                float(cfg["a22_buy_budget"]) / _a_ref * float(cfg["high_buy_max_price"]),
                float(cfg["b15_buy_budget"]) / _b_ref * float(cfg["buy_max_price"]),
                float(cfg["c5_buy_budget"]) / _c_ref * float(cfg["high_buy_max_price"]),
                _market_cap,
            )
            if float(cfg["buy_max_spend"]) + 1e-9 < _worst_spend:
                raise ValueError(
                    "buy_max_spend must cover slice_budget/ref * fak_limit "
                    "when size_to_ref_price is on (raise with budgets)"
                )
        ...
        for key in (
            ...
            "hedge_dump_persist_s", "hedge_dump_mid_ttm_s", "hedge_dump_mid_persist_s", "hedge_dump_late_ttm_s", "hedge_dump_late_persist_s",
            ...
            "early_rich_a22_ask_min", "early_rich_a22_ask_max", "early_rich_a22_min_edge_usd",
            "early_rich_take_profit_bid", "early_rich_take_profit_persist_s",
        ):
            if float(cfg[key]) < 0:
                raise ValueError(f"{key} must be non-negative")
        _eh_frac = float(cfg.get("early_hot_a22_fraction", 0.5) or 0.5)
        if not (0 < _eh_frac <= 1):
            raise ValueError("early_hot_a22_fraction must satisfy 0 < fraction <= 1")
        for _bk in (
        ...
        for key in (
            "hedge_dump_price_floor",
            "hedge_dump_ladder_start",
            "hedge_dump_ladder_step",
            "hedge_dump_ladder_step_sleep_s",
            "hedge_dump_late_sweep_ttm_s",
        ):
            if float(cfg.get(key, 0) or 0) < 0:
                raise ValueError(f"{key} must be non-negative")
        ...
        # Fail closed for THIS cycle only. Do not pin _strat_mtime to the bad
        # load — otherwise a transient validation/parse error permanently
        # disarms entries until the file mtime changes (Sep15 15:39 live).
        return _entries_disabled(_strat_cache or _STRATEGY_DEFAULTS)
    _strat_cache = cfg
    _strat_mtime = mtime
    return cfg
...
EARLY_HOT_DEFER_ENABLED = bool(_strat.get("early_hot_defer_enabled", True))
EARLY_HOT_DEFER_TTM_MIN = float(_strat.get("early_hot_defer_ttm_min", 15.0) or 0.0)
EARLY_HOT_DEFER_ASK_MIN = float(_strat.get("early_hot_defer_ask_min", 0.95) or 0.0)
...
EARLY_RICH_A22_ASK_MAX = float(_strat.get("early_rich_a22_ask_max", 0.98) or 0.0)
...
EARLY_RICH_A22_MIN_EDGE_USD = float(_strat.get("early_rich_a22_min_edge_usd", 60.0) or 0.0)
EARLY_RICH_TAKE_PROFIT_ENABLED = bool(_strat.get("early_rich_take_profit_enabled", True))
EARLY_RICH_TAKE_PROFIT_BID = float(_strat.get("early_rich_take_profit_bid", 0.99) or 0.0)
EARLY_RICH_TAKE_PROFIT_PERSIST_S = float(_strat.get("early_rich_take_profit_persist_s", 5.0) or 0.0)
...
HEDGE_DUMP_PERSIST_S = float(_strat.get("hedge_dump_persist_s", 0.0) or 0.0)
HEDGE_DUMP_MID_TTM_S = float(_strat.get("hedge_dump_mid_ttm_s", 300.0) or 0.0)
HEDGE_DUMP_MID_PERSIST_S = float(_strat.get("hedge_dump_mid_persist_s", 6.0) or 0.0)
HEDGE_DUMP_LATE_TTM_S = float(_strat.get("hedge_dump_late_ttm_s", 60.0) or 0.0)
HEDGE_DUMP_LATE_PERSIST_S = float(_strat.get("hedge_dump_late_persist_s", 2.0) or 0.0)
HEDGE_EDGE_COLLAPSE_PEAK_USD = float(_strat.get("hedge_edge_collapse_peak_usd", 40.0) or 0.0)
HEDGE_EDGE_COLLAPSE_NOW_USD = float(_strat.get("hedge_edge_collapse_now_usd", 10.0) or 0.0)
HEDGE_DUMP_REQUIRE_TIGHT = bool(_strat.get("hedge_dump_require_tight", True))
HEDGE_DUMP_IGNORE_SPREAD_ASK_MAX = float(_strat.get("hedge_dump_ignore_spread_ask_max", 0.60))
...
HEDGE_DUMP_LATE_SWEEP_TTM_S = float(
    _strat.get("hedge_dump_late_sweep_ttm_s", 45.0) or 0.0
)
...
TAKE_PROFIT_ENABLED = bool(_strat.get("take_profit_enabled", False))
TAKE_PROFIT_EDGE = float(_strat.get("take_profit_edge", 0.04) or 0.0)
TAKE_PROFIT_FROM_START_TS = float(_strat.get("take_profit_from_start_ts", 0.0) or 0.0)
...
SOFT_EDGE_EXIT_PERSIST_S = float(_strat.get("soft_edge_exit_persist_s", 2.0) or 0.0)
SOFT_EDGE_EXIT_QUOTE_MAX_AGE_S = float(
    _strat.get("soft_edge_exit_quote_max_age_s", 1.5) or 0.0
)
...
SIZE_TO_REF_PRICE = bool(_strat.get("size_to_ref_price", True))
A22_SIZE_REF_PRICE = float(_strat.get("a22_size_ref_price", 0.0) or 0.0)
B15_SIZE_REF_PRICE = float(_strat.get("b15_size_ref_price", 0.0) or 0.0)
C5_SIZE_REF_PRICE = float(_strat.get("c5_size_ref_price", 0.0) or 0.0)
...
LOG_BACKUP_COUNT = 30  # keep more local rotates; Drive/log_archive is long-term SoT
...
_hedge_dump_armed = {}  # cond -> monotonic ts when bid first <= toxic dump
_take_profit_persist_armed = {}  # cond -> monotonic ts when bid first >= TP target
_soft_edge_exit_armed = {}  # cond -> monotonic ts when soft-edge exit bid armed
_entry_book_persist_armed = {}  # cond|leg|band -> monotonic ts when book first ok
_depth_path_buffers = {}  # cond|leg -> DepthPathBuffer
_depth_path_last_sample_mono = {}  # cond|leg -> mono ts of last sample
_depth_topup_last_emit_mono = {}  # cond|leg -> mono ts of last topup emit
...
def dump_persist_for_ttm(minutes_left) -> float:
    """Dump persist seconds: shorten near expiry so wire cliffs can still fire.

    Tiers (live): default → mid (last 5m / 6s) → late (last 1m / 2s).
    Late is checked first so the tightest window wins.
    """
    try:
        ttm_s = float(minutes_left) * 60.0
    except (TypeError, ValueError):
        return float(HEDGE_DUMP_PERSIST_S)
    late_s = float(HEDGE_DUMP_LATE_TTM_S or 0)
    if late_s > 0 and ttm_s <= late_s + 1e-12:
        return float(HEDGE_DUMP_LATE_PERSIST_S)
    mid_s = float(HEDGE_DUMP_MID_TTM_S or 0)
    if mid_s > 0 and ttm_s <= mid_s + 1e-12:
        return float(HEDGE_DUMP_MID_PERSIST_S)
    return float(HEDGE_DUMP_PERSIST_S)


def hold_while_oracle_agrees(held_leg, start_ts, condition_id, meta=None):
    """True = do not sell; last live BTC is still with the bag (or feed is unread).

    Tracks peak favor-edge on ``meta``. If the edge collapsed from a large
    peak to a small still-winning residual, allow the sell/dump path
    (10pm-style cliff where oracle_still_winning_small blocked too long).
    """
    ...
    try:
        if edge is not None:
            e = float(edge)
            if math.isfinite(e):
                favor = e if str(held_leg).lower() == "up" else -e
    except (TypeError, ValueError):
        favor = None
    if meta is not None and favor is not None and favor > 0:
        try:
            peak = float(meta.get("oracle_peak_favor_edge") or 0)
        except (TypeError, ValueError):
            peak = 0.0
        if favor > peak + 1e-12:
            meta["oracle_peak_favor_edge"] = round(float(favor), 2)
    if allow:
        return False
    # Hard edge collapse while still slightly favoring held → allow sell/dump.
    try:
        peak = float((meta or {}).get("oracle_peak_favor_edge") or 0)
    except (TypeError, ValueError):
        peak = 0.0
    now_favor = float(favor) if favor is not None else None
    peak_need = float(HEDGE_EDGE_COLLAPSE_PEAK_USD or 0)
    now_max = float(HEDGE_EDGE_COLLAPSE_NOW_USD or 0)
    if meta is not None:
        meta.pop("_edge_collapse_unlock", None)
    if (
        peak_need > 0
        and now_max > 0
        and peak + 1e-12 >= peak_need
        and now_favor is not None
        and 0 < now_favor <= now_max + 1e-12
        and why in ("oracle_still_winning_small", "oracle_still_winning")
    ):
        log_buy_skip_throttled(
            "oracle_edge_collapse",
            condition_id,
            event="hedge_oracle_edge_collapse",
            leg=held_leg,
            ptb=uchk.get("ptb"),
            live_btc=uchk.get("live_btc"),
            edge_usd=None if edge is None else round(float(edge), 2),
            favor_edge=round(float(now_favor), 2),
            peak_favor_edge=round(float(peak), 2),
            peak_need_usd=peak_need,
            now_max_usd=now_max,
            oracle_why=why,
            favored=uchk.get("favored"),
            allows_dump=bool(HEDGE_EDGE_COLLAPSE_ALLOWS_DUMP),
        )
        if meta is not None:
            meta["_edge_collapse_unlock"] = True
        return False
    _hedge_persist_armed.pop(condition_id, None)
    _hedge_persist_done.discard(condition_id)
    _hedge_dump_armed.pop(condition_id, None)
    event = (
    ...
    log_buy_skip_throttled(
        ...
        feed=uchk.get("feed"),
        peak_favor_edge=None if meta is None else meta.get("oracle_peak_favor_edge"),
    )
    return True
...
def current_entry_bands(minutes_left):
    ...
    return applicable_hourly_entry_bands(
        ...
        early_rich_ask_max=EARLY_RICH_A22_ASK_MAX,
    )
    ...
    try:
        ...
        if not levels:
            ...
            try:
                ...
                levels = []
        ladder = compute_depth_ladder(levels, limit_price, budgets=budgets)
        payload = {
            "condition_id": condition_id,
            "token_id": token_id,
            "slug": slug,
            ...
            "limit": round(float(limit_price), 4) if limit_price is not None else None,
            "ttm": ttm,
            "attempt": attempt,
            "outcome": outcome,
            ...
            "top_asks": ladder.get("top_asks") or summarize_ask_levels(levels, n=5),
        }
        log_event("buy_depth_ladder", **{
            k: v for k, v in payload.items() if v is not None
        })
        append_depth_ladder_jsonl(DEPTH_LADDER_JSONL, {
            "event": "buy_depth_ladder",
            **payload,
        })
        # Ring-buffer sample + top-up path sim (research only; no trade effect).
        gates_for_sample = outcome not in ("no_ask", None)
        record_depth_path_sample(
            condition_id=condition_id,
            token_id=token_id,
            slug=slug,
            leg=leg,
            ...
            ttm=ttm,
            gates_ok=gates_for_sample,
            asks=levels,
            force=True,
        )
        emit_buy_depth_topup_sim(
            condition_id=condition_id,
            token_id=token_id,
            slug=slug,
            leg=leg,
...
def _depth_path_key(condition_id=None, leg=None, token_id=None):
    """Stable ring-buffer key: prefer condition|leg, else token_id."""
...
def _depth_path_buffer(key, *, maxlen=DEPTH_PATH_BUFFER_MAX):
    buf = _depth_path_buffers.get(key)
    if buf is None:
        buf = DepthPathBuffer(maxlen=maxlen)
        _depth_path_buffers[key] = buf
    return buf


def prune_depth_path_buffers(keep_tokens=None):
    """Drop path buffers whose token is no longer watched (market rollover)."""
    keep = {str(t) for t in (keep_tokens or []) if t}
    if not keep:
        # Cap memory if caller has no keep set.
        if len(_depth_path_buffers) > 64:
            for stale in list(_depth_path_buffers.keys())[:32]:
                _depth_path_buffers.pop(stale, None)
                _depth_path_last_sample_mono.pop(stale, None)
                _depth_topup_last_emit_mono.pop(stale, None)
        return
    for key in list(_depth_path_buffers.keys()):
        # Keys are condition|leg or bare token_id. Keep condition keys while
        # any watched token exists; drop bare-token keys not in keep.
        if "|" not in key and key not in keep:
            _depth_path_buffers.pop(key, None)
            _depth_path_last_sample_mono.pop(key, None)
            _depth_topup_last_emit_mono.pop(key, None)
    if len(_depth_path_buffers) > 128:
        # Drop oldest-sample keys first.
        ordered = sorted(
            _depth_path_buffers.items(),
            key=lambda kv: _depth_path_last_sample_mono.get(kv[0], 0.0),
        )
...
def record_depth_path_sample(
    *,
    ...
    asks=None,
    force=False,
    min_interval_s=DEPTH_PATH_SAMPLE_MIN_INTERVAL_S,
):
    """Append one cheap depth sample to the per-leg ring buffer (telemetry only).

    Reuses already-fetched REST ask levels when fresh; otherwise TOB ask*size.
    Does not issue new REST calls.
    """
    try:
        key = _depth_path_key(condition_id, leg, token_id)
        if not key:
            return None
        now_mono = time.monotonic()
        if not force:
            prev = _depth_path_last_sample_mono.get(key, 0.0)
            if now_mono - prev < float(min_interval_s):
                return None
        limit = finite_float(limit_price, minimum=0, maximum=1)
        if limit is None:
            return None
        levels = asks
        if levels is None and token_id is not None:
            levels = get_cached_ask_levels(token_id, max_age_s=5.0)
        if ask_size is None and token_id is not None:
            # Prefer size already in WS/REST quote caches — no HTTP.
            try:
                wq = book_ws.quote(token_id, max_age_s=2.5) if book_ws else None
            except Exception:
                wq = None
            if wq is not None and len(wq) >= 4:
                ask_size = wq[3]
            if ask_size is None:
                cached = _rest_quote_cache.get(token_id) or _rest_quote_cache.get(str(token_id))
                if cached is not None and isinstance(cached, tuple) and cached[0] is not None:
                    q = cached[0]
        ...
        if gates_ok is None:
            ...
            if bid is not None or ask is not None:
                book_ok, _why = entry_book_ok(
                    bid, ask, MAX_ENTRY_SPREAD, MIN_WINNER_BID,
                )
            gates_ok = bool(in_band and book_ok)
        ...
        buf = _depth_path_buffer(key)
        buf.append(sample)
        _depth_path_last_sample_mono[key] = now_mono
        return sample
    except Exception as exc:
        try:
            log_event(
                "buy_depth_topup_sample_fail",
                token_id=token_id,
                condition_id=condition_id,
                error=str(exc)[:200],
            )
...
def emit_buy_depth_topup_sim(
    *,
    ...
    min_interval_s=DEPTH_TOPUP_EMIT_MIN_INTERVAL_S,
):
    """Emit cumulative top-up path sim from the ring buffer (telemetry only)."""
    try:
        key = _depth_path_key(condition_id, leg, token_id)
        if not key:
            return None
        ...
        now_mono = time.monotonic()
        if not force:
            prev = _depth_topup_last_emit_mono.get(key, 0.0)
            if now_mono - prev < float(min_interval_s):
                return None
        samples = buf.samples()
        sim = simulate_topup_path(samples, targets=targets)
        limit = finite_float(limit_price, minimum=0, maximum=1)
        if limit is None and samples:
            limit = finite_float(samples[-1].get("limit"), minimum=0, maximum=1)
        payload = {
            "condition_id": condition_id,
            "token_id": token_id,
            "slug": slug,
        ...
        append_depth_ladder_jsonl(DEPTH_TOPUP_JSONL, {
            "event": "buy_depth_topup_sim",
            **payload,
        })
        _depth_topup_last_emit_mono[key] = now_mono
        return payload
    except Exception as exc:
        try:
            log_event(
                "buy_depth_topup_sim_fail",
                token_id=token_id,
                condition_id=condition_id,
                error=str(exc)[:200],
            )
...
def maybe_sample_depth_path_leg(
    *,
    ...
    """Cheap main-loop sample when we already have a quote for a buy-window leg."""
    if ask is None or not bands or not token_id:
        return
    in_band = ask_in_any_band(ask, bands)
    book_ok, _why = entry_book_ok(bid, ask, MAX_ENTRY_SPREAD, MIN_WINNER_BID)
    band = select_hourly_entry_band(ask, bands) if in_band else None
    if band is not None:
        limit = float(band.fak_limit)
        slice_name = band.name
    else:
        try:
            limit = max(float(b.fak_limit) for b in bands)
        except Exception:
            return
        slice_name = None
    record_depth_path_sample(
        ...
        ask_size=ask_size,
        limit_price=limit,
        ttm=round(float(minutes_left), 2) if minutes_left is not None else None,
        gates_ok=bool(in_band and book_ok),
    )
    emit_buy_depth_topup_sim(
        condition_id=condition_id,
...
def prune_rest_caches(keep_tokens=None):
    ...
    keep = {str(t) for t in (keep_tokens or []) if t}
    try:
        prune_depth_path_buffers(keep_tokens=keep_tokens)
    except Exception:
        pass
    if keep:
...
def remember_held_quote(token_id, bid, ask):
    if token_id is None or bid is None:
        return
    _last_good_held_quote[str(token_id)] = (bid, ask, time.monotonic())
...
def last_good_held_age(token_id):
    """Seconds since remember_held_quote; None if never stored."""
    rec = _last_good_held_quote.get(str(token_id or ""))
    if not rec or len(rec) < 3 or rec[2] is None:
        return None
    try:
        age = time.monotonic() - float(rec[2])
    except (TypeError, ValueError):
        return None
    if age < 0:
        return None
...
def buy_exec_tick(tick_size):
    """CLOB tick for this buy FAK. Same floor as hedge — never post finer than market min."""
    return hedge_market_tick(tick_size, EXPECTED_TICK_SIZE)
...
def buy_limit_price(limit, tick_size):
    """FAK buy limit, rounded *up* to the market tick so the order still crosses.

    WS/books can advertise millitick levels while CLOB signing requires 0.01
    (live hourly 12 Sep 4pm a22: ``invalid tick size (0.001), minimum is 0.01``).
    Sells floor via hedge_sell_price; buys ceil so a 0.987 ask-cap becomes 0.99.
    """
    tick = float(buy_exec_tick(tick_size))
    raw = float(limit or 0)
    if raw <= 0:
        return tick
    aligned = math.ceil(raw / tick - 1e-12) * tick
    if aligned > 1.0 + 1e-12:
        aligned = math.floor(1.0 / tick + 1e-12) * tick
    return max(tick, min(1.0, round(aligned, 10)))
...
def entry_persist_clamp_for_ttm(persist_s, minutes_left):
    """If TTM < required persist, shorten so a last-minute in-band print can clear.

    Uses frac*TTM with a floor, capped so we still leave ~2s of wire before expiry.
    """
    try:
        wait = float(persist_s or 0.0)
    except (TypeError, ValueError):
    ...
    try:
        ...
        return wait
    if not (ttm_s > 0) or ttm_s + 1e-12 >= wait:
        return wait
    frac = float(ENTRY_PERSIST_TTM_FRAC or 0.5)
    if frac <= 0:
        frac = 0.5
    floor_s = float(ENTRY_PERSIST_TTM_FLOOR_S or 0.0)
    clamped = max(floor_s, frac * ttm_s)
    # Never demand more hold than remaining time minus a small wire buffer.
    wire_room = max(0.0, ttm_s - 2.0)
    return min(wait, clamped, wire_room) if wire_room > 1e-12 else min(wait, max(0.5, 0.5 * ttm_s))

def entry_book_persist_key(cond, leg, band=None):
    """Persist arm key: separate timers per entry band/level (b15 vs a22 vs c5)."""
    band_s = str(band or "").strip() or "none"
    return f"{cond}|{leg}|{band_s}"


def clear_entry_book_persist_leg(cond, leg):
    """Drop every band-level persist arm for this condition|leg."""
    prefix = f"{cond}|{leg}|"
    for key in list(_entry_book_persist_armed):
        if key.startswith(prefix) or key == f"{cond}|{leg}":
            _entry_book_persist_armed.pop(key, None)


def entry_book_persist_ready(cond, leg, book_ok, *, now_s=None, persist_s=None, band=None):
    """Require entry_book_ok to hold for persist_s before BUY.

    Arms are keyed by condition|leg|band so a b15 (~90–94¢) timer cannot
    satisfy a later a22 (≥95¢) buy on the same leg.
    ...
    if wait <= 1e-12:
        return True, "ok", 0.0
    armed = _entry_book_persist_armed.get(key)
    if armed is None:
        _entry_book_persist_armed[key] = float(now_s)
...
def hourly_size_ref_price(slice_name, *, size_to_ref=True, a22_ref=0.0, b15_ref=0.0,
                          c5_ref=0.0, a22_min=0.95, buy_threshold=0.90, c5_min=0.95):
    """Reference price for share-target sizing, or None when disabled.

    Prefer deriving from band floors (``a22_min_price`` / ``buy_threshold`` /
    ``c5_min_price``). Explicit ``*_size_ref_price`` > 0 overrides (e.g. 0.95).
    """
    if not size_to_ref:
        return None
    name = str(slice_name or "")
    if name == "a22":
        ref = float(a22_ref or 0.0) or float(a22_min or 0.0)
    elif name == "b15":
        ref = float(b15_ref or 0.0) or float(buy_threshold or 0.0)
    elif name == "c5":
        ref = float(c5_ref or 0.0) or float(c5_min or 0.0)
    else:
        return None
    ref = finite_float(ref, minimum=0, maximum=1)
    if ref is None or ref <= 0:
        return None
    return float(ref)


def target_buy_shares(budget, ref_price):
    """Ideal share count: ``budget / ref_price`` (before 2dp / exact-cent snap)."""
    budget = finite_float(budget, minimum=0)
    ref = finite_float(ref_price, minimum=0, maximum=1)
    if budget is None or ref is None or budget < 0.01 or ref <= 0:
        return 0.0
    return float(Decimal(str(budget)) / Decimal(str(ref)))


def quoted_buy_shares_up_to_limit(
    budget, ask, limit, share_cap=None, spend_cap=None, ref_price=None,
):
    """Shares for a FAK at ``limit``.

    Default (no ``ref_price``): maximize spend ≤ budget/spend_cap at limit
    (exact-cent maker, 2 dp shares).

    With ``ref_price`` > 0 (share-target): hit ~``budget / ref_price`` shares so
    modeled win size stays stable when limit/ask > ref. Spend ceiling is
    ``max(budget, target_shares * limit)``, also clipped by ``spend_cap`` /
    ``share_cap``. Exact-cent snap at limit may land slightly under the ideal
    target (e.g. 0.99 only allows integer shares).
    """
    _ = ask  # call-site compat; FAK size is limit-based
    limit_f = finite_float(limit, minimum=0, maximum=1)
    if limit_f is None or limit_f <= 0:
        return 0.0
    budget_f = finite_float(budget, minimum=0)
    if budget_f is None or budget_f < 0.01:
        return 0.0
    ref = finite_float(ref_price, minimum=0, maximum=1)
    if ref is not None and ref > 0:
        target = Decimal(str(budget_f)) / Decimal(str(ref))
        desired = target * Decimal(str(limit_f))
        ceiling = max(Decimal(str(budget_f)), desired)
        cap_arg = finite_float(spend_cap, minimum=0)
        if cap_arg is not None and cap_arg >= 0.01:
            ceiling = min(ceiling, Decimal(str(cap_arg)))
    else:
    ...
    cap_d = ceiling.quantize(cent, rounding=ROUND_DOWN)
    if cap_d < cent:
        return 0.0
    ...
    while sh >= min_shares:
        maker = sh * lim
        if maker.quantize(cent) == maker and maker <= cap_d:
            return float(sh)
        sh -= share_tick
...
def classify_buy_fill(avg, filled, quoted_shares, min_price, toxic_below):
    ...
    Unphysical averages (None / >1), e.g. leftover ``pnl_entry_cost`` after a
    half take-profit before cost is reduced, must not clamp to 0 and false-arm
    toxic dump.
    """
    avg = finite_float(avg, minimum=0, maximum=1)
    if avg is None:
        return False, False
...
_HEDGE_UNCERTAIN_KEYS = (
    ...
    "hedge_uncertain_is_take_profit",
...
def buy_market_with_retry(
    ...
    depth_ttm=None,
    size_ref_price=None,
    market_spend_remaining=None,
):
    """Buy token_id via FAK, sized in shares at the band limit.

    Posts a **limit** FAK at the **open band max** (hourly B 90¢, A/C 99¢).
    Maker USDC is exact cents (2 dp) at that limit — otherwise CLOB FAK BUY
    returns 400 ``invalid amounts``. This is not a USDC market order.

    With ``size_ref_price`` (share-target): size ~``budget / ref`` shares so
    win payout stays stable when limit > ref; spend may slightly exceed the
    nominal slice budget (clipped by ``BUY_MAX_SPEND`` /
    ``market_spend_remaining``). Without ref: maximize spend ≤ budget at
    limit. ``buy_max_shares`` is the per-FAK rail.

    Band is still min_price–max_price for *whether* to fire. The limit is
    max_price so the FAK can take depth behind the touch.
    ...
    spent = 0.0
    budget = float(budget)
    # Honor coarser CLOB minimum (WS may cache 0.001 while market min is 0.01).
    tick_size = str(buy_exec_tick(tick_size))
    ...
    for attempt in range(max_retries):
        ...
        per_fak_cap = float(BUY_MAX_SPEND)
        if market_spend_remaining is not None:
            try:
                per_fak_cap = min(per_fak_cap, float(market_spend_remaining))
            except (TypeError, ValueError):
                pass
        ref = finite_float(size_ref_price, minimum=0, maximum=1)
        if ref is not None and ref > 0:
            # Share-target: do not clip spend_cap to nominal slice budget —
            # target_shares * limit may slightly exceed budget when limit > ref.
            spend_ceiling = max(0.0, per_fak_cap - spent)
            shares = quoted_buy_shares_up_to_limit(
                remaining_budget,
                fresh_ask,
                float(max_price),
                BUY_MAX_SHARES,
                spend_cap=spend_ceiling,
                ref_price=float(ref),
            )
        else:
            shares = quoted_buy_shares_up_to_limit(
                ...
                float(max_price),
                BUY_MAX_SHARES,
                spend_cap=max(0.0, min(per_fak_cap, float(budget)) - spent),
            )
        if shares < 0.01:
            break
        limit_price = buy_limit_price(float(max_price), tick_size)
        price = limit_price
        spend = shares * price
        max_shares = shares
    ...
    for attempt in range(max_retries):
        ...
        if use_price_ladder:
            ...
            if ladder_levels:
                ...
                # Refresh terminal rung with current live bid (ladder may
                # already step below floor toward live_bid).
            ...
            try:
                ...
                if meta.get("hedge_uncertain"):
                    ...
                    if hedge_state == "confirmed":
                        ...
                        # Pro-rata leftover cost from pre-sell snapshot (fixes
                        # ambiguous→exact_order half-TP leaving VWAP ~1.9).
                        try:
                            cost0 = float(
                                meta.get("hedge_uncertain_entry_cost")
                                if meta.get("hedge_uncertain_entry_cost") is not None
                                else (meta.get("pnl_entry_cost") or 0)
                            )
                            if position_size > 0.01 and rem < position_size and cost0 > 0:
                                meta["pnl_entry_cost"] = round(
                                    cost0 * (rem / position_size), 4
                                )
                        except (TypeError, ValueError, ZeroDivisionError):
                            pass
                        if meta.get("hedge_uncertain_is_take_profit") and rem > 0.01:
                ...
                if held_size > 0.01 and not meta.get("hedge_closed"):
                    ...
                    # Skip unphysical leftovers (e.g. full cost / half size after
                    # TP before pro-rata fix, or any avg outside [0,1]).
                    if 0 < float(implied or 0) <= 1.0 + 1e-9:
                        _, force_exit = classify_buy_fill(
                ...
                # --- TAKE PROFIT (oracle-bypass; next-hour+ bags only) ---
                # Soft-edge paranoia exit (Joel): soft entry favor-edge bags
                # that print a rich held bid get a full discretionary sell
                # every tick — constant in-bot watch without waiting on me.
                if (
                    bool(held_token)
                    and held_size > 0.01
                    and not meta.get("hedge_closed")
                    and not meta.get("soft_edge_exit_done")
                    and SOFT_EDGE_EXIT_ENABLED
                    and float(SOFT_EDGE_EXIT_BID or 0) > 1e-12
                    and float(SOFT_EDGE_EXIT_MAX_USD or 0) > 1e-12
                ):
                    try:
                        _se_entry = float(meta.get("entry_favor_edge_usd"))
                    except (TypeError, ValueError):
                        _se_entry = None
                    if _se_entry is None:
                        try:
                            _raw = float(meta.get("entry_edge_usd"))
                            if math.isfinite(_raw):
                                _se_entry = _raw if held_leg == "up" else -_raw
                        except (TypeError, ValueError):
                            _se_entry = None
                    if (
                        _se_entry is not None
                        and math.isfinite(_se_entry)
                        and 0 < float(_se_entry) <= float(SOFT_EDGE_EXIT_MAX_USD) + 1e-12
                    ):
                        # Soft-edge uses its own quote max-age (default 1.5s),
                        # not hedge_quote_max_age_s (0.25s). Winner bags never
                        # reach remember_held_quote on the hedge path
                        # (oracle_still_winning), so requiring ultra-fresh WS
                        # alone reset the persist arm before 2s could elapse.
                        _se_max_age = float(SOFT_EDGE_EXIT_QUOTE_MAX_AGE_S or 0.0)
                        if _se_max_age <= 1e-12:
                            _se_max_age = float(HEDGE_QUOTE_MAX_AGE_S)
                        _se_age = book_ws.quote_age(held_token)
                        _se_bid = _se_ask = None
                        if _se_age is not None and _se_age <= _se_max_age:
                            _se_q = book_ws.quote(
                                held_token, max_age_s=_se_max_age,
                            )
                            if _se_q is not None:
                                _se_bid = _se_q[0]
                                _se_ask = _se_q[2]
                        if _se_bid is not None:
                            remember_held_quote(held_token, _se_bid, _se_ask)
                        else:
                            _lg_bid, _lg_ask = last_good_held_quote(held_token)
                            _lg_age = last_good_held_age(held_token)
                            if (
                                _lg_bid is not None
                                and _lg_age is not None
                                and _lg_age <= _se_max_age + 1e-12
                            ):
                                _se_bid = _lg_bid
                                _se_ask = _lg_ask
                        _se_ok = (
                            _se_bid is not None
                            and float(_se_bid) + 1e-12 >= float(SOFT_EDGE_EXIT_BID)
                        )
                        # Keep the armed timer across a brief quote gap when
                        # last-good is still rich and within soft max-age
                        # (do not force a re-arm from zero).
                        if (
                            not _se_ok
                            and _soft_edge_exit_armed.get(cond) is not None
                        ):
                            _lg_bid2, _ = last_good_held_quote(held_token)
                            _lg_age2 = last_good_held_age(held_token)
                            if (
                                _lg_bid2 is not None
                                and _lg_age2 is not None
                                and _lg_age2 <= _se_max_age + 1e-12
                                and float(_lg_bid2) + 1e-12
                                >= float(SOFT_EDGE_EXIT_BID)
                            ):
                                _se_bid = _lg_bid2
                                _se_ok = True
                        _se_now = time.monotonic()
                        _se_fire, _se_armed, _se_why = hedge_persist_ready(
                            bool(_se_ok),
                            now_s=_se_now,
                            armed_ts=_soft_edge_exit_armed.get(cond),
                            persist_s=float(SOFT_EDGE_EXIT_PERSIST_S),
                            toxic=False,
                        )
                        if _se_armed is None:
                            _soft_edge_exit_armed.pop(cond, None)
                        else:
                            _soft_edge_exit_armed[cond] = _se_armed
                        if _se_ok and not _se_fire:
                            log_buy_skip_throttled(
                                _se_why,
                                cond,
                                event="soft_edge_exit_persist_wait",
                                leg=held_leg,
                                bid=round(float(_se_bid), 4),
                                entry_favor_edge_usd=round(float(_se_entry), 2),
                                exit_bid=float(SOFT_EDGE_EXIT_BID),
                                persist_s=float(SOFT_EDGE_EXIT_PERSIST_S),
                            )
                        if _se_fire:
                            _soft_edge_exit_armed.pop(cond, None)
                            _se_tick = hedge_exec_tick(
                                get_tick_size_cached(held_token),
                            )
                            _se_floor = float(_se_tick)
                            _se_px = hedge_sell_price(
                                _se_bid, _se_tick, 0, _se_floor,
                            )
                            log_event(
                                "soft_edge_exit_sell",
                                condition_id=cond,
                                ...
                                ttm=round(float(minutes_left), 2),
                            )
                            # Reuse dump-style full sell (oracle bypass).
                            sold, sell_res, hedge_proceeds = sell_market_with_retry(
                                ...
                                max_spread=None,
                                condition_id=cond,
                                initial_quote=(_se_bid, None),
                                dump=True,
                                persist_done=True,
                                market_tick=_se_tick,
                                max_retries=12,
                            ...
                            if sold and float(sold) > 0.01:
                                rem = max(0.0, float(held_size) - float(sold))
                                meta["bought_size"] = rem
                                if rem <= 0.01:
                                    meta["hedge_closed"] = True
                                    meta["soft_edge_exit_done"] = True
                                ...
                                prior = float(meta.get("pnl_hedge_proceeds") or 0)
                                meta["pnl_hedge_proceeds"] = round(
                                    prior + float(hedge_proceeds or 0), 4
                                )
                                save_json(STATE_FILE, positions_meta)
                                log_event(
                                    "soft_edge_exit_filled",
                                    condition_id=cond,
                                    leg=held_leg,
                                    sold=float(sold),
                                    proceeds=float(hedge_proceeds or 0),
                                    remaining=rem,
                                    entry_favor_edge_usd=round(float(_se_entry), 2),
                ...
                # Sell when live bid >= blended cost (pnl_entry_cost/bought_size,
                # else fill_price) + take_profit_edge AND bid has stayed
                # qualifying for take_profit_persist_s (hedge_persist_ready).
                # MUST bypass hold_while_oracle_agrees — winning bags never
                ...
                if (
                    ...
                    and take_profit_market_armed(
                        m.start_ts, TAKE_PROFIT_FROM_START_TS,
                    )
                    and (
                        not meta.get("take_profit_done")
                        or float(TAKE_PROFIT_FULL_BID or 0) > 1e-12
                        or (
                            bool(meta.get("early_rich_a22"))
                            and bool(EARLY_RICH_TAKE_PROFIT_ENABLED)
                            and float(EARLY_RICH_TAKE_PROFIT_BID or 0) > 1e-12
                        )
                    )
                ):
                    _tp_age = book_ws.quote_age(held_token)
                    _tp_bid = _tp_ask = None
                    if _tp_age is not None and _tp_age <= float(HEDGE_QUOTE_MAX_AGE_S):
                        _tp_q = book_ws.quote(
                            held_token, max_age_s=float(HEDGE_QUOTE_MAX_AGE_S),
                        )
                        if _tp_q is not None:
                            _tp_bid = _tp_q[0]
                            _tp_ask = _tp_q[2]
                    if _tp_bid is None:
                        _tp_bid, _tp_ask2 = last_good_held_quote(held_token)
                        if _tp_ask is None:
                            _tp_ask = _tp_ask2
                    if _tp_bid is None:
                        _tp_rest_b, _, _tp_rest_a, _, _ = get_quote_fast(
                            held_token,
                            max_age_s=0.0,
                            prefer_rest=True,
                            force_rest=True,
                        ...
                        if _tp_rest_b is not None:
                            _tp_bid = _tp_rest_b
                        if _tp_ask is None and _tp_rest_a is not None:
                            _tp_ask = _tp_rest_a
                    _tp_vwap = blended_cost_per_share(
                        meta.get("pnl_entry_cost"),
                        held_size,
                        meta.get("fill_price"),
                    )
                    # Early-rich bags: full exit at early_rich_take_profit_bid
                    # (live 0.99); leave TAKE_PROFIT_FULL_BID for normal bags.
                    _tp_er = bool(meta.get("early_rich_a22")) and bool(
                        EARLY_RICH_TAKE_PROFIT_ENABLED
                    )
                    _tp_full_bid_eff = (
                        float(EARLY_RICH_TAKE_PROFIT_BID)
                        if _tp_er else float(TAKE_PROFIT_FULL_BID or 0)
                    )
                    _tp_persist_eff = (
                        float(EARLY_RICH_TAKE_PROFIT_PERSIST_S)
                        if _tp_er else float(TAKE_PROFIT_PERSIST_S)
                    )
                    _tp_full_lock = take_profit_full_ready(
                        _tp_bid, _tp_full_bid_eff,
                    )
                    # Early-rich bags: never edge-TP below the 99¢ floor —
                    # wait for bid ≥ early_rich_take_profit_bid.
                    _tp_edge_ok = False if _tp_er else take_profit_overrides_oracle(
                        _tp_bid,
                        _tp_vwap,
                        TAKE_PROFIT_EDGE,
                        enabled=True,
                    ...
                    if (
                        _tp_er
                        and not _tp_full_lock
                        and _tp_bid is not None
                    ):
                        log_buy_skip_throttled(
                            "early_rich_tp_bid_soft",
                            cond,
                            event="take_profit_skip_early_rich_bid_soft",
                            leg=held_leg,
                            bid=round(float(_tp_bid), 4),
                            tp_bid=float(_tp_full_bid_eff),
                            vwap=(
                                None if _tp_vwap is None
                                else round(float(_tp_vwap), 4)
                            ),
                        )
                    ...
                    if meta.get("take_profit_done") and not _tp_full_lock:
                        ...
                        _tp_qualifies = bool(_tp_full_lock or _tp_edge_ok)
                    _tp_frac = (
                    ...
                    if _tp_qualifies and not _tp_fire:
                        ...
                        log_buy_skip_throttled(
                            _tp_pwhy,
                            cond,
                            event="take_profit_persist_wait",
                            leg=held_leg,
                            bid=(
                                None if _tp_bid is None
                                else round(float(_tp_bid), 4)
                            ),
                            vwap=(
                                None if _tp_vwap is None
                                else round(float(_tp_vwap), 4)
                            ...
                            persist_s=float(_tp_persist_eff),
                    ...
                    if _tp_fire:
                        _take_profit_persist_armed.pop(cond, None)
                        _tp_tick = hedge_exec_tick(
                            get_tick_size_cached(held_token),
                        )
                        _tp_floor = float(_tp_tick)
                        if _tp_full_lock:
                            _tp_target = float(_tp_full_bid_eff)
                        else:
                            _tp_target = float(_tp_vwap) + float(TAKE_PROFIT_EDGE)
                        _tp_sell_px = hedge_sell_price(
                            _tp_bid, _tp_tick, 0, _tp_floor,
                        )
                        # Early-rich: never post sell limit below TP bid.
                        if _tp_er:
                            _tp_sell_px = max(
                                float(_tp_sell_px),
                                float(_tp_full_bid_eff),
                            )
                        _tp_sell_size = take_profit_sell_size(
                            held_size, _tp_frac,
                        )
                        if _tp_sell_size < 0.01:
                            log_event(
                                "take_profit_skip_size",
                                condition_id=cond,
                                leg=held_leg,
                                size=held_size,
                        ...
                        _tp_bag = live_bag_log_fields(
                            ...
                            ask=(
                                None if _tp_ask is None
                                else round(float(_tp_ask), 4)
                            ),
                            tick=_tp_tick,
                        )
                        log_event(
                            ...
                            start_ts=float(m.start_ts),
                            **_tp_bag,
                        )
                        console.print(Panel(
                            "  [bright_white]%s[/]\n"
                            "  [bright_green]TAKE PROFIT[/] — %s "
                            "bid [bold]%.3f[/] >= "
                            "vwap %.3f+%.2f "
                            "(>=%.3f)  ·  "
                            "sell %.2f/%.2f (%.0f%%)  ·  "
                            "FAK >=%.3f  ·  "
                            "[bold green]TTM %4.1fm[/]"
                            % (
                                m.question,
                                held_leg.upper(),
                                float(_tp_bid),
                                float(_tp_vwap),
                                float(TAKE_PROFIT_EDGE),
                                _tp_target,
                                _tp_sell_size,
                                held_size,
                                float(_tp_frac) * 100.0,
                                _tp_sell_px,
                                minutes_left,
                            ),
                            title="[bold bright_green]▲ TAKE PROFIT — LOCKING GAINS[/]",
                            border_style="bright_green",
                            box=box.HEAVY,
                        ))
                        prior_tp_proceeds = float(
                            meta.get("pnl_hedge_proceeds", 0) or 0
                        )

                        def _clear_tp_uncertain():
                            clear_uncertain_fields(
                                meta, _HEDGE_UNCERTAIN_KEYS,
                            )

                        def _abort_tp_submit():
                            _clear_tp_uncertain()
                            save_json(STATE_FILE, positions_meta)

                        def _persist_tp_submit(
                            ...
                            meta["hedge_uncertain_confirmed_proceeds"] = float(
                                proceeds_total
                            )
                            ...
                            meta["hedge_uncertain_position_size"] = float(held_size)
                            meta["hedge_uncertain_pnl_before"] = float(
                                prior_tp_proceeds
                            )
                            meta["hedge_uncertain_is_take_profit"] = True
                            meta["hedge_uncertain_entry_cost"] = float(
                                meta.get("pnl_entry_cost") or 0
                            )
                            save_json(STATE_FILE, positions_meta)

                        def _persist_tp_fill(sold_total, proceeds_total):
                            ...
                            if should_mark_hedge_closed(sold_total, rem_now):
                                ...
                                _take_profit_persist_armed.pop(cond, None)
                                _hedge_persist_done.discard(cond)
                            if float(sold_total or 0) > 0.01:
                                meta["take_profit_done"] = True
                            # Pro-rata from pre-TP snapshot so callback + later
                            # reconcile stay idempotent (no double-shrink).
                            prior_size = float(
                                meta.get("hedge_uncertain_position_size")
                                or held_size
                                or 0
                            ...
                            meta["pnl_hedge_proceeds"] = round(
                                prior_tp_proceeds + float(proceeds_total), 4
                            )
                            meta["hedge_uncertain_confirmed_sold"] = float(sold_total)
                            meta["hedge_uncertain_confirmed_proceeds"] = float(
                                proceeds_total
                        ...
                        def _tp_pre_submit(live_bid, _live_ask, _attempt_no):
                            # Take-profit intentionally skips oracle veto.
                            if live_bid is None:
                                return False, "no_bid"
                            if float(live_bid) + 1e-12 < float(_tp_target):
                                return False, "bid_below_take_profit"
                            return True, "take_profit_oracle_bypass"
                        ...
                        log_event(
                            ...
                            edge=float(TAKE_PROFIT_EDGE),
                            target=round(_tp_target, 4),
                            price_limit=_tp_sell_px,
                            **_tp_bag,
                        )
                        sold, sell_res, tp_proceeds = sell_market_with_retry(
                            held_token,
                            _tp_sell_size,
                            _tp_bid,
                            tick_size=_tp_tick,
                            ...
                            require_ask_max=None,
                            max_spread=None,
                            on_submit=_persist_tp_submit,
                            on_abort=_abort_tp_submit,
                            on_fill=_persist_tp_fill,
                            condition_id=cond,
                            initial_quote=(_tp_bid, _tp_ask),
                            dump=False,
                            persist_done=False,
                            pre_submit=_tp_pre_submit,
                            deadline_ts=float(m.end_ts),
                        )
                        sell_status = (sell_res or {}).get("bot_status")
                        last_limit = (sell_res or {}).get("last_limit", _tp_sell_px)
                        order_error = (sell_res or {}).get("order_error")
                        if sell_status in ("ambiguous", "persist_fail"):
                            log_event(
                                "take_profit_uncertain",
                                condition_id=cond,
                                leg=held_leg,
                                sell_status=sell_status,
                            ...
                            continue
                        api_bal = check_clob_token_balance(
                            held_token, refresh=True,
                        )
                        rec = reconcile_hedge_sold(
                            held_size, sold, tp_proceeds, api_bal, last_limit,
                        )
                        rem = rec["rem"]
                        ...
                        if rec["effective_sold"] > 0.01:
                            meta["take_profit_done"] = True
                            meta["pnl_hedge_proceeds"] = round(
                                prior_tp_proceeds + rec["proceeds"], 4
                            )
                            meta["bought_size"] = rem
                            try:
                                prior_size = float(
                                ...
                                if prior_size > 1e-12 and rem < prior_size:
                                    meta["pnl_entry_cost"] = round(
                                        prior_cost * (rem / prior_size), 4
                                    )
                            except (TypeError, ValueError, ZeroDivisionError):
                                pass
                            leg_key = "up" if held_leg == "up" else "dn"
                            ...
                            save_json(STATE_FILE, positions_meta)
                            notify(
                                "TAKE PROFIT" if rem < 0.01 else "TAKE PROFIT PARTIAL",
                                "Take-profit on %s\nSold %s at ~%.3f (%.2f sh, rem %.2f; vwap %.3f+%.2f)"
                                % (
                                    m.question,
                                    held_leg.upper(),
                                    fill_px,
                            ...
                            log_event(
                                ...
                                sell_status=sell_status,
                                order_error=order_error,
                                vwap=None if _tp_vwap is None else round(float(_tp_vwap), 4),
                                edge=float(TAKE_PROFIT_EDGE),
                                **_tp_bag,
                            )
                            save_json(STATE_FILE, positions_meta)
                ...
                # Exception: held bid <= toxic dump (live 35c) may override
                # oracle after dump_persist_s (live 8s).
                toxic_oracle_override = False
                edge_collapse_unlock = False
                _dump_max_age = float(HEDGE_DUMP_MAX_QUOTE_AGE_S or 0.0)
                if _dump_max_age <= 1e-12:
                    _dump_max_age = float(HEDGE_QUOTE_MAX_AGE_S)
                if hedge_open and float(HEDGE_TOXIC_BID_MAX) > 1e-12:
                    _oq_age = book_ws.quote_age(held_token) if held_token else None
                    _obid = _oask = None
                    # Toxic oracle override requires a fresh book (WS or
                    # last_good within hedge_dump_max_quote_age_s) — never
                    # a stale last_good alone.
                    if _oq_age is not None and _oq_age <= _dump_max_age + 1e-12:
                        _oq = book_ws.quote(
                            held_token, max_age_s=_dump_max_age,
                        )
                        if _oq is not None:
                            _obid = _oq[0]
                            _oask = _oq[2]
                    if _obid is None:
                        _lg_age_ov = last_good_held_age(held_token)
                        if (
                            _lg_age_ov is not None
                            and _lg_age_ov <= _dump_max_age + 1e-12
                        ):
                            _obid, _oask2 = last_good_held_quote(held_token)
                            if _oask is None:
                                _oask = _oask2
                    if (
                        _obid is not None
                        and float(_obid) <= float(HEDGE_TOXIC_BID_MAX) + 1e-12
                    ):
                        if HEDGE_DUMP_REQUIRE_TIGHT:
                            if (
                                _oask is not None
                                and (float(_oask) - float(_obid))
                                <= float(HEDGE_MAX_SPREAD) + 1e-12
                            ):
                                toxic_oracle_override = True
                        else:
                            toxic_oracle_override = True
                if (
                    hedge_open
                    and (not toxic_oracle_override)
                    and hold_while_oracle_agrees(held_leg, m.start_ts, cond, meta)
                ):
                    hedge_open = False
                elif hedge_open and (not toxic_oracle_override):
                ...
                if hedge_open:
                    ...
                    last_good_age = last_good_held_age(held_token)
                    persist_done = cond in _hedge_persist_done
                    armed_ts = _hedge_persist_armed.get(cond)
                    # Edge-collapse unlocks normal hedge only unless knob
                    # explicitly allows dump (bid_le_dump).
                    dump_bid_cap = float(HEDGE_TOXIC_BID_MAX)
                    if (
                    ...
                    if recovered or winner_before_persist:
                        ...
                        toxic_dump_on = dump_bid_cap > 1e-12
                        peek_dump = (
                            toxic_dump_on
                            and peek_bid is not None
                            and peek_bid <= dump_bid_cap + 1e-12
                        )
                        last_dump = (
                            toxic_dump_on
                            and last_bid is not None
                            and last_bid <= dump_bid_cap + 1e-12
                        )
                        # last_dump alone may skip REST only when last_good
                        # is within dump max age (never 64–91s stale).
                        last_dump_fresh = (
                            last_dump
                            and last_good_age is not None
                            and last_good_age <= _dump_max_age + 1e-12
                        )
                        if peek_dump or last_dump_fresh or persist_done:
                            need_rest = False
                        # Dump path always prefers a fresh REST when WS is
                        # stale and last_good is not dump-fresh.
                        if (
                            HEDGE_DUMP_REQUIRE_FRESH_BOOK
                            and toxic_dump_on
                            and (not ws_dump_fresh)
                            and (not last_dump_fresh)
                            and (peek_dump or last_dump)
                        ):
                            need_rest = True
                        # Even when dump_bid_cap==0 (edge-collapse), a
                        # ≤toxic last_good/peek must not skip REST — otherwise
                        # sell_fade can fire on a 64–91s ghost bid.
                        _peek_toxic = (
                            float(HEDGE_TOXIC_BID_MAX) > 1e-12
                            and peek_bid is not None
                            and float(peek_bid)
                            <= float(HEDGE_TOXIC_BID_MAX) + 1e-12
                        )
                        ...
                        if (
                            HEDGE_DUMP_REQUIRE_FRESH_BOOK
                            and (not ws_dump_fresh)
                            and (not _last_toxic_fresh)
                            and (_peek_toxic or _last_toxic)
                        ):
                            need_rest = True
                        ...
                        preview = evaluate_held_bag(
                            ...
                            dump_persist_s=dump_persist_for_ttm(minutes_left),
                        ...
                        # Toxic dump 8s arm (separate from qualify persist).
                        dts = getattr(intent, "dump_armed_ts", None)
                        if intent.dump and dts is not None:
                            _hedge_dump_armed[cond] = dts
                        elif not intent.dump:
                            _hedge_dump_armed.pop(cond, None)

                        # Dump arm/fire freshness: WS age ≤ max OR this-tick
                        # REST succeeded. Refuse dump on stale last_good alone.
                        ...
                        if (
                            ...
                            dump_book_fresh = True
                        # Toxic-priced exit (≤ dump cap) needs a fresh book
                        # whether classified as dump *or* sell_fade. Demoting
                        # dump→sell_fade on stale last_good was the 8am Sep15
                        # hole when persist_done / edge-collapse unlocked.
                        _toxic_px = (
                            hedge_bid is not None
                            and float(HEDGE_TOXIC_BID_MAX) > 1e-12
                            and float(hedge_bid)
                            <= float(HEDGE_TOXIC_BID_MAX) + 1e-12
                        )
                        _toxic_exit = _toxic_px and intent.action in {
                            "dump", "sell", "arm", "wait",
                        }
                        if (
                            HEDGE_DUMP_REQUIRE_FRESH_BOOK
                            ...
                            log_buy_skip_throttled(
                                "dump_stale_quote",
                                cond,
                                event="hedge_skip_dump_stale_quote",
                                **bag_log,
                                quote_age_s=(
                                    None if quote_age is None else round(quote_age, 3)
                                ),
                                last_good_age_s=(
                                    None
                                    if last_good_age is None
                                    else round(last_good_age, 3)
                                ),
                                dump_max_quote_age_s=_dump_max_age,
                                rest_ok=rest_bid is not None,
                                intent_action=intent.action,
                                edge_collapse_unlock=bool(edge_collapse_unlock),
                            )
                            _hedge_dump_armed.pop(cond, None)
                            _hedge_persist_armed.pop(cond, None)
                            # HOLD — do not demote into sell_fade at ghost bid.
                            intent = HedgeIntent(
                                "hold", "dump_stale_blocked", None, None,
                                False, True, None, False, None,
                            )

                        # Refuse toxic DUMP while oracle still favors held.
                        # Also refuse toxic-priced sell_fade under the same
                        # favor gate (dump_bid_cap=0 demotion / edge-collapse).
                        if (
                            _toxic_exit
                            and intent.action in {"dump", "sell", "arm", "wait"}
                        ):
                            _df_favor = None
                            try:
                                ...
                                _df_favor = None
                            # Dump always favor-gated. Toxic sell/arm while
                            # dump is disabled (edge-collapse) also gated so
                            # soft-winning oracle cannot panic-exit at ≤toxic.
                            _favor_gate = bool(intent.dump) or (
                                dump_bid_cap <= 1e-12
                            ...
                            if (
                                ...
                                log_buy_skip_throttled(
                                    "dump_oracle_favor",
                                    cond,
                                    event="hedge_skip_dump_oracle_favor",
                                    **bag_log,
                                ...
                                _hedge_persist_armed.pop(cond, None)
                                intent = HedgeIntent(
                                    "hold", "dump_favor_blocked", None, None,
                                    False, True, None, False, None,
                                )

                        # edge_collapse_allows_dump=false → no economic dump
                        # via sell_fade at ≤ toxic (even on a fresh book).
                        if (
                            edge_collapse_unlock
                            and (not HEDGE_EDGE_COLLAPSE_ALLOWS_DUMP)
                            and _toxic_px
                            ...
                            log_buy_skip_throttled(
                                "dump_collapse_no_dump",
                                cond,
                                event="hedge_skip_dump_collapse_disabled",
                                **bag_log,
                            ...
                            _hedge_persist_armed.pop(cond, None)
                            intent = HedgeIntent(
                                "hold", "collapse_dump_disabled", None, None,
                                False, True, None, False, None,
                            )

                        if intent.reason in {"dump_wide_spread", "dump_missing_ask"}:
                            log_buy_skip_throttled(
                                intent.reason,
                                cond,
                                event="hedge_skip_dump_book",
                                **bag_log,
                                max_spread=HEDGE_MAX_SPREAD,
                        ...
                        if intent.action in {"arm", "wait"}:
                            ...
                            def _hedge_pre_submit(_live_bid, _live_ask, _attempt_no):
                                ...
                                # Toxic dump (<=35c persist) overrides oracle;
                                # normal <=60c scratch still needs oracle against.
                                if dump:
                                    return True, "dump_oracle_override"
                                if hold_while_oracle_agrees(
                                    held_leg, m.start_ts, cond, meta,
                                ):
                ...
                if not ENTRY_ENABLED:
                    log_buy_skip_throttled(
                        "entry_disabled", cond, event="buy_skip",
                ...
                _arm_kwargs = dict(
                    ...
                    # Coarse gate: let early-hot partials reach the buy path so
                    # the later can_arm (with allow_a22_topup=not early_hot_active)
                    # can decide. Full budget so room for top-up is visible.
                    allow_a22_topup=bool(
                        meta.get("a22_early_hot_partial")
                        and not meta.get("a22_early_hot_complete")
                    ),
                    a22_full_budget=A22_BUY_BUDGET,
                )
                _any_slice_ok = False
                _arm_reasons = []
                for _band in bands:
                ...
                # Depth top-up path samples (telemetry): reuse quotes already in hand.
                try:
                ...
                if up_buy:
                    ...
                    if not ready:
                        log_buy_skip_throttled(
                            "entry_book_persist",
                            cond,
                            event="buy_skip_entry_book_persist",
                            leg="up",
                            band=_up_band_name,
                            persist_why=why,
                ...
                # Early-rich a22 needs a stronger oracle floor than normal buys
                # (default $60 vs min_underlying_edge_usd $40) so a soft edge
                # cannot arm on sticky >=97c alone.
                if (
                    _early_rich_open
                    ...
                    _er_edge = uchk.get("edge_usd")
                    if _er_edge is not None:
                        _er_favor = float(_er_edge) if favored == "up" else -float(_er_edge)
                        _er_ask = up_ask if up_buy else dn_ask
                        _er_min = float(EARLY_RICH_A22_MIN_EDGE_USD or 0.0)
                        if (
                            _er_min > 0
                            and _er_ask is not None
                            and float(_er_ask) + 1e-12 >= float(EARLY_RICH_A22_ASK_MIN)
                            and _er_favor + 1e-12 < _er_min
                        ):
                            log_buy_skip_throttled(
                                "early_rich_edge",
                                cond,
                                event="buy_skip_early_rich_edge",
                                favor_edge=round(float(_er_favor), 2),
                                min_edge_usd=_er_min,
                                ask=round(float(_er_ask), 4),
                                ask_min=EARLY_RICH_A22_ASK_MIN,
                                favored=favored,
                                minutes_left=round(float(minutes_left), 2),
                                ptb=uchk.get("ptb"),
                ...
                # Early-hot: TTM > threshold + expensive ask + large oracle
                ...
                if (
                    ...
                    and favored
                    and uchk is not None
                    and minutes_left > EARLY_HOT_DEFER_TTM_MIN
                ):
                    _edge = uchk.get("edge_usd")
                    if _edge is not None:
                        favor_edge = float(_edge) if favored == "up" else -float(_edge)
                        ask = up_ask if up_buy else dn_ask
                        if (
                            ask is not None
                            and float(ask) >= EARLY_HOT_DEFER_ASK_MIN
                            and favor_edge >= EARLY_HOT_DEFER_EDGE_USD
                        ):
                            early_hot_active = True
                            early_hot_favor_edge = favor_edge
                            early_hot_ask = float(ask)
                _eh_frac = float(EARLY_HOT_A22_FRACTION or 0.5)
                if _eh_frac <= 0 or _eh_frac > 1:
                    _eh_frac = 0.5
                # If b15 never filled, fold its budget into a22 so a late
                # rich ask (no 90s print) still targets ~a22+b15. Early-hot
                # first clip stays half of base a22; full/top-up includes orphan.
                _a22_full_with_orphan = a22_budget_with_orphan_b15(
                    meta, float(A22_BUY_BUDGET), float(B15_BUY_BUDGET),
                )
                effective_a22_budget = (
                    float(A22_BUY_BUDGET) * _eh_frac
                    if early_hot_active
                    else float(_a22_full_with_orphan)
                )
                _arm_kwargs["a22_budget"] = effective_a22_budget
                _arm_kwargs["allow_a22_topup"] = not early_hot_active
                _arm_kwargs["a22_full_budget"] = float(_a22_full_with_orphan)
                if early_hot_active:
                    log_buy_skip_throttled(
                        "early_hot_half_cap",
                        cond,
                        event="early_hot_half_cap",
                        minutes_left=round(float(minutes_left), 2),
                        ask=round(float(early_hot_ask), 4),
                        favor_edge=round(float(early_hot_favor_edge), 2),
                        favored=favored,
                        a22_fraction=round(_eh_frac, 4),
                        a22_effective_budget=round(float(effective_a22_budget), 4),
                        a22_full_budget=round(float(_a22_full_with_orphan), 4),
                        a22_orphan_b15=round(float(_a22_full_with_orphan) - float(A22_BUY_BUDGET), 4),
                        a22_spent_usd=round(float(meta.get("a22_spent_usd") or 0), 4),
                        a22_early_hot_partial=bool(meta.get("a22_early_hot_partial")),
                        allow_b15=bool(EARLY_HOT_ALLOW_B15),
                        ttm_min=EARLY_HOT_DEFER_TTM_MIN,
                        ask_min=EARLY_HOT_DEFER_ASK_MIN,
                        edge_usd_min=EARLY_HOT_DEFER_EDGE_USD,
                        ptb=uchk.get("ptb") if uchk else None,
                        live_btc=uchk.get("live_btc") if uchk else None,
                        up_ask=up_ask,
                        dn_ask=dn_ask,
                    )
                ...
                spend_usd = hourly_slice_budget(
                    band.name, meta,
                    a22_budget=effective_a22_budget,
                    b15_budget=B15_BUY_BUDGET,
                    c5_budget=C5_BUY_BUDGET,
                    market_cap=MARKET_SPEND_CAP,
                )
                ...
                if spend_usd < 0.01:
                    continue
                _a22_early_hot_cap_buy = (
                    band.name == "a22" and early_hot_active
                )
                _a22_topup_buy = (
                    band.name == "a22"
                    and (not early_hot_active)
                    and bool(meta.get("a22_early_hot_partial"))
                    and not bool(meta.get("a22_early_hot_complete"))
                )
                _a22_spent_before = float(meta.get("a22_spent_usd") or 0)
                ...
                if _early_rich_fire and buy_ask is not None:
                    _er_tp_cap = float(EARLY_RICH_TAKE_PROFIT_BID or 0.0)
                    _er_ask_cap = float(EARLY_RICH_A22_ASK_MAX or 0.0)
                    if _er_ask_cap <= 0:
                        _er_ask_cap = max(0.0, _er_tp_cap - 0.01) if _er_tp_cap > 0 else 0.98
                    if (
                        (
                            _er_tp_cap > 1e-12
                            and float(buy_ask) + 1e-12 >= _er_tp_cap
                        )
                        or (
                            _er_ask_cap > 1e-12
                            and float(buy_ask) - 1e-12 > _er_ask_cap
                        )
                    ):
                        log_buy_skip_throttled(
                            "early_rich_ask_too_rich",
                            cond,
                            event="buy_skip_early_rich_ask_too_rich",
                            leg=buy_leg,
                            ask=round(float(buy_ask), 4),
                            ask_max=round(float(_er_ask_cap), 4),
                            tp_bid=round(float(_er_tp_cap), 4),
                            band=band.name,
                            minutes_left=round(float(minutes_left), 2),
                        )
                        continue
                ...
                def _persist_buy_fill(filled_total, spent_total):
                    ...
                    if band.name == "a22":
                        ...
                        if _a22_early_hot_cap_buy:
                            meta["a22_early_hot_partial"] = True
                        if _a22_topup_buy:
                ...
                def _buy_pre_submit(fresh_bid, fresh_ask, _attempt_no):
                    ...
                    selected_book_ok, selected_book_why = entry_book_ok(
                        fresh_bid, fresh_ask, MAX_ENTRY_SPREAD, MIN_WINNER_BID,
                    )
                    # Fail closed on wide/missing book at POST — do not rely only on
                    # hourly_entry_final_gate's generic book_not_winner label.
                    if not selected_book_ok:
                        return False, str(selected_book_why or "wide_spread")
                ...
                _size_ref = hourly_size_ref_price(
                    band.name,
                    size_to_ref=SIZE_TO_REF_PRICE,
                    a22_ref=A22_SIZE_REF_PRICE,
                    b15_ref=B15_SIZE_REF_PRICE,
                    c5_ref=C5_SIZE_REF_PRICE,
                    a22_min=A22_MIN_PRICE,
                ...
                bought, spent, buy_status = buy_market_with_retry(
                    ...
                    depth_leg=buy_leg,
                    depth_slice=band.name,
                    depth_ttm=round(minutes_left, 2),
                    size_ref_price=_size_ref,
                    market_spend_remaining=hourly_remaining_to_cap(
                        meta, MARKET_SPEND_CAP,
                    ),
                )
                ...
                if bought > 0:
                    ...
                    meta["quoted_buy_shares_total"] = float(total_quoted or 0)
                    meta["toxic_fill"] = bool(force_exit or meta.get("toxic_fill"))
                    # Fresh sleeve after a prior full hedge must be hedgeable.
                    if meta.get("hedge_closed") and float(slice_prior_size or 0) <= 0.01:
                        meta["hedge_closed"] = False
                        meta.pop("take_profit_done", None)
                        meta["pnl_hedge_proceeds"] = float(
                    ...
                    if uchk:
                        ...
                        try:
                            _ee = float(uchk.get("edge_usd"))
                            if math.isfinite(_ee):
                                meta["entry_favor_edge_usd"] = round(
                                    _ee if buy_leg == "up" else -_ee, 2
                                )
                        except (TypeError, ValueError):
                            pass
                    log_event(
                        ...
                        a22_spent_usd=meta.get("a22_spent_usd"),
