...
`scrapbidder.py` + `deploy/polyscrapbid.service` buy from the complement deposit wallet. Cap is **20 shares** and **`bid_max_px` 4¢** (~$0.80), not a sell and not a price we always pay. After A `sold_loser` on leg L, B immediately rests a BUY at **4¢** (`bid_max_px`). A's scrap rest and the 180s window do not block that leg. There is no cheaper rest and no price ladder. FAK is used only when `shares * price` ≥ `bid_fak_min_notional` (~$1); 20 shares at 4¢ stays a resting order. GTD is used when the expiration is at least ~180s ahead; otherwise the order is GTC and still cancelled by T−20s / window end. Never above 4¢. The other leg (winner A still holds) stays blocked. Markets A never held still wait for the last 180s and a cheap book, then rest at the same 4¢ cap. If that sold leg has no B bid or fill for ~10s while the window is open past cancel, log `scrapbid_miss` (throttled ~30s) and poll at ~1s until the order is up. Books are quoted only for open sister orders, the last 180s, or a sold leg, and mint intents are read again after that quote so a scrap during the pass is not planned as `a_still_long`. Never mint. Never FAK-sell. `bid_enabled` false and `dry_run` true until the operator turns them on. Credentials stay in gitignored `.env.complement`. Do not start `polyscrapbid` unless the operator asks. This is not `complementbot`.
