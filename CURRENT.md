...
`scrapbidder.py` + `deploy/polyscrapbid.service` rest **BUY** limits from the complement deposit wallet. Default **20 shares at 4¢** (`bid_max_px=0.04`). That 4¢ is a buy, not a sell. Once A has `sold_loser` on leg L, B bids L immediately, even if A's scrap rest is still live and even when TTM is longer than 180s. Cancel by T−20s / window end. The other leg (winner A still holds) stays blocked. Markets A never held still wait for the last 180s and a cheap book. If that sold leg has no B bid for ~10s while the window is open past cancel, log `scrapbid_miss` (throttled ~30s) and poll at ~1s until the bid is up. Never mint. Never FAK-sell. `bid_enabled` false and `dry_run` true until the operator turns them on. Credentials stay in gitignored `.env.complement`. Do not start `polyscrapbid` unless the operator asks. This is not `complementbot`.
