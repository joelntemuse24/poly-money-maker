#!/usr/bin/env python3
"""Now-snapshot: would the 5m $5 dry-run probe fire on the current clock?

Public Gamma + CLOB GET only. Does not import buybot5m.py, does not read .env,
does not POST orders. Default strategy is strategy_buy5m_probe.example.json.
"""
...
DEFAULT_STRATEGY = REPO / "strategy_buy5m_probe.example.json"
...
def _book(token_id: str) -> Dict[str, Any]:
    response = requests.get(
        ...
        timeout=10,
        headers={"User-Agent": "poly-money-maker-5m-probe-now/1.0"},
    )
...
def _decide(cfg: Dict[str, Any], seconds_left: float, up: Dict[str, Any], dn: Dict[str, Any]) -> Dict[str, Any]:
    window_s = float(cfg.get("buy_start_s") or 0)
    ...
    window_ok = in_buy_window_s(seconds_left, window_s)
    ...
    if not (up_ok or dn_ok):
        reasons.append("ask_not_in_ge975_band")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument(
        ...
        help="probe JSON (default: strategy_buy5m_probe.example.json)",
    )
    ...
    print(
        ...
        f"window={cfg.get('buy_start_s')}s "
    ...
    if not live:
        print("no active btc-up-or-down-5m market")
