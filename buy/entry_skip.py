...
def applicable_hourly_entry_bands(
    ...
    **> a22_min** (live ~0.949). FAK **99¢**.
    ...
    (live 0.97) up to ``early_rich_ask_max`` (live 0.98; defaults to
    ``min(high_max, 0.98)`` when omitted) — strictly below the early-rich
    take-profit bid so a 99¢ buy cannot lock flat/negative vs a 99¢ TP.
    This is an extra gate so a sticky 97¢+ book can arm a22 beside b15
    before the last-10m window. It does not raise last-10m a22's floor
    or persist.
    ...
    if a22_open:
        ...
        try:
            ...
            er_min = 0.97
        try:
            if early_rich_ask_max is None or early_rich_ask_max == "":
                er_max = min(float(high_max), 0.98)
            else:
                er_max = float(early_rich_ask_max)
        except (TypeError, ValueError):
            er_max = min(float(high_max), 0.98)
        if er_max + 1e-12 < er_min:
            er_max = er_min
        if c5_open:
            er_max = min(er_max, float(c5_min))
        bands.append(EntryBand(
            float(er_min), float(er_max), False, HOURLY_SLICE_A, float(er_max),
        ))
    if c5_open:
        bands.append(EntryBand(
