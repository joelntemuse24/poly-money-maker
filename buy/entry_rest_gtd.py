...
# Exact now+180 still comes back rejected on the wire.
# Signing, HTTP, and any venue security haircut eat the boundary.
CLOB_GTD_LEAD_SLACK_S = 90
...
def gtd_expiration(
    ...
    try:
        ...
        now_s = time.time()
    slack = int(CLOB_GTD_LEAD_SLACK_S) if int(CLOB_GTD_LEAD_SLACK_S) > 0 else 0
    floor = int(now_s) + lead + slack
