...
**hedges**: it market-sells the held leg when bid ≤ 65¢ (and ask ≤ 70¢ with a
...
**Economics per share (no reversal):** buy at ~98–99¢, redeem at $1.00 → ~1–2¢
gross.
...
2. **GUI consensus.** The Polymarket UI display price (mid when spread ≤ 10¢, else
   last trade) must show a clear winner and loser: winner ≥ `min_winner_bid` (92¢),
...
4. **Ask band.** The best ask of the winning leg is within
   `buy_threshold`–`buy_max_price` (98–99¢). The bot buys as soon as ask ≥ 98¢
   (other gates passing) and posts the FAK at the *live* ask. 99¢ is the hard
   ceiling (never enter if ask is already above it).
5. **Underlying gate.** If `underlying_gate_enabled`, the live oracle price must be
   at least `min_underlying_edge_usd` away from the captured PTB (probe uses **$0**),
...
|---|---|---|
| `buy_threshold` / `buy_max_price` | 0.98 / 0.99 | Trigger ≥98¢; hard ceiling 99¢ |
...
| `underlying_gate_enabled` / `min_underlying_edge_usd` | true / 0.0 | Oracle alignment gate (direction only) |
| `toxic_force_exit_below` | 0.90 | Force-dump if FAK avg &lt; 90¢ (must be ≤ buy_threshold) |
...
journal is capped via `deploy/journald-size.conf`.
