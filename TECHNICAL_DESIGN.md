...
`deploy/polyscrapbid.service` is opt-in and stays disabled. It runs `scrapbidder.py` with `EnvironmentFile=.env.complement` only (not mintbot `.env`). Wallet B buys, cap 20 shares at `bid_max_px` 0.04. After A `sold_loser` on leg L it rests a BUY at 4¢ immediately (A's scrap rest and the 180s window do not block it). No cheaper rest and no escalate ladder. FAK only when notional ≥ ~$1. GTD when expiration is ≥ ~180s ahead, else GTC, still cancelled by T−20s. Markets A never held rest at that cap on the cheap live side only in the last 180s. It does not mint and does not FAK-sell. `bid_enabled` defaults false and `dry_run` defaults true. Do not commit `.env.complement`. Same-wallet buyback is not implemented. Do not enable this unit unless the operator asks.
