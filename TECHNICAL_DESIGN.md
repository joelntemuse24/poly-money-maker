...
- FAK ladder walks every 1¢ from `sell_fak_px` (**0.02**) down to `sell_floor` (**0.02**), inclusive. Those rungs are equal, so a book at or above the floor posts a single 2¢ limit, clamped to the live sized bid. A thinner book starts at the live bid and continues 1¢ at a time to the floor. A bid below the floor is the only rung. The ladder does not post above the fak rung. If displayed depth at the rung is positive and shorter than our size, the FAK clips to that depth (`loser_partial_fak_shares`). Empty depth does not clip.
...
|---|---|---|
| Loser | Arm ≤ 0.02. Ladder walks every 1¢ from `sell_fak_px` down to `sell_floor` (both 0.02, so one rung), clamped to the live bid (below floor: that bid only). Rest after a miss at `min(sell_scrap_rest_px, live or last-seen bid)`. Blind empty FAK at 0.01 | 2¢ starts the hunt and is the print ceiling. A wider print-to-floor gap (live 3¢ → 1¢) tries each cent on the same fire. A thinner book, including the post-miss rest, posts the live bid. GTD only when expiry is ≥ ~180s ahead. |
