...
|---|---|---|
| Loser | Arm ≤ 0.02. Ladder `sell_fak_px` 0.02 (equal to the floor), or live bid when thinner (below floor: that bid only). Rest after a miss at `min(sell_scrap_rest_px, live or last-seen bid)`. Blind empty FAK at 0.01 | 2¢ starts the hunt and is the print ceiling. A thinner book, including the post-miss rest, posts the live bid. GTD only when expiry is ≥ ~180s ahead. |
