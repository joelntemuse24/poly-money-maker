...
- **Late-window oracle veto is off.** `sell_late_window_s` is **0**, and `sell_oracle_edge_floor_usd`, `sell_oracle_edge_per_ttm`, `sell_oracle_edge_persist_s`, and `sell_oracle_stale_s` are **0**. Zero window skips the block: `manage_sells` only enters it when the window is positive and TTM is inside it. The zero edge knobs are what shipped so a later window edit does not bring back the old $25 / 1.5×TTM / 3s / 5s-stale rule. The helper still implements that rule when those arguments are passed explicitly. With these defaults, CLOB gates only. `oracle_log_enabled` stays on.
