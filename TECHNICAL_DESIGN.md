...
`.env.complement`. CLOB clients (API-key derive **and** trading) pass
the same `signature_type=1` + `funder` as 15m `buybot.py` so orders
use the deposit/proxy maker flow. A complement POST is never treated
