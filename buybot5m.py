...
CHAIN_ID = 137
STATE_FILE = "positions_buy5m.json"
...
_STRATEGY_DEFAULTS = {
    ...
    "hedge_enabled": True,
    # Persist 1s @ 50/52 then sell at the live bid while it is still
    ...
    # Walk flatten: toxic bags sell at the live bid while bid < buy_threshold.
    # Separate from dump_bid_max so 40 < 50 <= 53 still holds.
...
def load_strategy():
    ...
    try:
        ...
        if not (
            ...
            raise ValueError(
                "early_95_min_price must satisfy buy_max_price < min95 <= early_max <= 1"
            )
...
if DRY_RUN:
    STATE_FILE = "positions_buy5m.dryrun.json"
...
def meta_fill_band(meta):
    """Band used to classify this bag's average (latest slice wins)."""
    lo = finite_float((meta or {}).get("entry_band_min"), minimum=0, maximum=1)
    hi = finite_float((meta or {}).get("entry_band_max"), minimum=0, maximum=1)
    if lo is not None and hi is not None:
        return lo, hi
    if (meta or {}).get("late_bought"):
        return float(BUY_THRESHOLD), float(BUY_MAX_PRICE)
    if (meta or {}).get("early_bought"):
        return float(BUY_MAX_PRICE), float(EARLY_BUY_MAX_PRICE)
    return float(BUY_THRESHOLD), float(BUY_MAX_PRICE)


def decide_entry_at(seconds_left, ask):
    return decide_5m_entry(
        seconds_left,
        ask,
        late_start_s=BUY_START_S,
        late_min=BUY_THRESHOLD,
...
def get_book_quote(token_id, expected_condition_id=None):
    ...
    try:
        ...
        _book_snapshot_meta[str(token_id)] = {
            ...
            "min_order_size": finite_float(
                book.get("min_order_size"), minimum=0,
            ),
...
def hedge_book_ok(bid, ask, threshold, max_spread, require_ask_max):
    ...
    Dump (bid ≤ 40) does not use this gate.
    """
...
def hedge_gui_limits(require_ask_max):
    ...
    stay bid-only at hedge_toxic_bid_max (40¢). Walk flatten is a
    separate bid < buy_threshold path.
    """
...
def classify_buy_fill(avg, filled, quoted_shares, min_price, toxic_below, max_price=None):
    ...
    Junk walks show up as ``avg < toxic_below``. A late 93¢ or early 85¢
    average is outside the band and dumps (22 Aug illegal late 91+).
    """
    below, above, toxic = classify_fill_against_band(
        avg, min_price, max_price, toxic_below,
    )
    return (below or above), toxic
...
_BUY_UNCERTAIN_KEYS = (
    ...
    "buy_uncertain_leg", "buy_uncertain_baseline", "buy_uncertain_attempt",
    "buy_uncertain_spend", "buy_uncertain_price", "buy_uncertain_order_id",
    "buy_uncertain_order_size", "buy_uncertain_trade_ids",
...
_HEDGE_UNCERTAIN_KEYS = (
    "hedge_uncertain", "hedge_uncertain_at", "hedge_uncertain_token",
    "hedge_uncertain_attempt", "hedge_uncertain_remaining",
    "hedge_uncertain_price", "hedge_uncertain_confirmed_sold",
    "hedge_uncertain_confirmed_proceeds", "hedge_uncertain_order_id",
    "hedge_uncertain_order_size", "hedge_uncertain_trade_ids",
    "hedge_uncertain_status",
    "hedge_uncertain_sold_before", "hedge_uncertain_proceeds_before",
    "hedge_uncertain_position_size", "hedge_uncertain_pnl_before",
)
...
def buy_market_with_retry(
    ...
    exceed ``buy_max_spend``, then ``clip_buy_shares_to_displayed_ask``
    caps to displayed best-ask size (fail-closed if missing/thin). The
    clip re-validates maker cents at the posted band-max limit.
    ...
    lifetime share limit. Displayed top size is a separate clip.
    ...
    for attempt in range(max_retries):
        ...
        shares = clip_buy_shares_to_displayed_ask(
            shares, fresh_ask, fresh_ask_size, float(max_price),
        )
        if shares < 0.01:
            console.print(
        ...
        below_band, force_exit = classify_buy_fill(
            avg, filled, max_shares, min_price, TOXIC_FORCE_EXIT_BELOW, max_price,
        )
        total_bought += filled
        spent += fill_cost
        remaining_budget = budget - spent
...
def sell_market_with_retry(
    ...
    try:
        ...
        dump_bid_max = 0.40
    try:
        flatten_ceil = float(BUY_THRESHOLD)
    except (NameError, TypeError, ValueError):
        flatten_ceil = 0.75
    try:
        qualify_bid = float(
...
banner = Panel(
    Align.center(
        ...
        "[bold bright_green]██╔══██╗   ██║   ██║     [/]   [bright_yellow]//[/]  [dim]BTC 5M · HEDGE @ 65¢[/]\n"
...
while not _shutdown_requested:
    try:
        ...
        # Each endpoint has its own worker so a slow Gamma/Data request cannot
        # delay a held-position hedge or starve another refresh category.
        if (
        ...
        if _open_pos_n and _show_ui:
            ...
            for cond, pos in held.items():
                try:
                    ...
                    if m:
                        secs = (m.end_ts * 1000 - now_ms) / 1000
                    else:
                        end_date = meta.get("end_date", "")
                        secs = -1
                        if end_date:
                            try:
                                ...
                                secs = (end_dt.timestamp() * 1000 - now_ms) / 1000
                            except Exception:
                                pass
        ...
        for m in _loop_markets:
            ...
            try:
                ...
                seconds_left = entry_seconds_left(
                    now_ms / 1000.0, m.end_ts, getattr(m, "slug", None), 300.0,
                )
                ...
                # Hedge the token that the position source says is actually held.
                # Gamma metadata may be stale or mismatched during market rollover.
                ...
                if meta.get("buy_uncertain") and uncertain_order_id:
                    ...
                    if inspected_state == "confirmed":
                        ...
                        band_lo, band_hi = meta_fill_band(meta)
                        _, force_exit = classify_buy_fill(
                            avg_fill, resolved_size, quoted,
                            band_lo, TOXIC_FORCE_EXIT_BELOW, band_hi,
                        )
                        ...
                        stamp_slice_on_inventory(
                            meta, bool(meta.get("buy_uncertain_late_slice")), resolved_size,
                        )
                        leg_key = (
                            "up"
                    ...
                    if recovered or winner_before_persist:
                        ...
                        if hedge_oracle_blocks_sell(
                            ...
                            if (
                                ...
                                and hedge_bid + 1e-12 >= flatten_max
                            ):
                                log_event(
                            ...
                            if dump and intent.reason == "flatten_walk":
                                hedge_label = "FLATTEN WALK <{:.0f}¢".format(
                                    flatten_max * 100
                                )
                            elif dump:
                            ...
                            def _hedge_pre_submit(_live_bid, _live_ask, _attempt_no):
                                ...
                                if dump and HEDGE_DUMP_IGNORE_ORACLE:
                                    return True, "ok"
                ...
                seconds_left = entry_seconds_left(
                    time.time(), m.end_ts, getattr(m, "slug", None), FIVE_M_DURATION_S,
                )
                late_slice = is_late_entry_window(seconds_left, BUY_START_S)
                bands = current_entry_bands(seconds_left)
                band = decide_entry_at(seconds_left, buy_ask)
                if band is None or not bands:
                    log_event(
                        "buy_skip",
                        condition_id=cond,
                        reason="ttm_crossed_rest",
                        ask=buy_ask,
                        seconds_left=round(seconds_left, 1),
                        slug=getattr(m, "slug", None),
                    )
                ...
                def _persist_buy_fill(filled_total, spent_total):
                    ...
                    _, force_exit = classify_buy_fill(
                        avg, filled_total, this_quoted,
                        band.min_price, TOXIC_FORCE_EXIT_BELOW, band.max_price,
                    )
                    ...
                    meta["entry_band_min"] = float(band.min_price)
                    meta["entry_band_max"] = float(band.max_price)
                    meta["toxic_fill"] = bool(force_exit or meta.get("toxic_fill"))
                    stamp_slice_on_inventory(meta, late_slice, filled_total)
                ...
                if bought > 0:
                    ...
                    meta["entry_band_max"] = float(band.max_price)
                    meta["toxic_fill"] = bool(force_exit or meta.get("toxic_fill"))
                    stamp_slice_on_inventory(meta, late_slice, bought)
                    if uchk:
