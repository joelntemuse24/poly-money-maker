...
class SweepTemplateTests(unittest.TestCase):
    ...
    def test_template_reads_hourly_example(self):
        ...
        self.assertEqual(tmpl["budget"], 5.0)
        self.assertEqual(tmpl["hedge_threshold"], 0.60)
        self.assertEqual(tmpl["hedge_require_ask_max"], 0.62)
        self.assertEqual(tmpl["hedge_toxic_bid_max"], 0.0)
    ...
    def test_sweep_starts_from_live_template(self):
        ...
        self.assertNotIn("window_120s", names)
        self.assertIn("window_180s", names)
        self.assertIn("window_45s", names)
        self.assertIn("window_60s", names)
        self.assertEqual(sweep_variants(tmpl)[0]["ttm_max"], 120.0)
