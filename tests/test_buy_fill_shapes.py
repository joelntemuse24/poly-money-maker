...
class BalanceAndGcSemantics(unittest.TestCase):
    ...
    def test_docs_vm_paste_is_last120_edge0(self):
        root = BOT5M.parent
        text = (root / "CURRENT.md").read_text()
        self.assertIn('d["buy_start_s"]=120', text)
        self.assertIn('d["early_buy_start_s"]=120', text)
        self.assertIn('d["early_95_start_s"]=0', text)
        self.assertIn('d["early_95_min_s"]=0', text)
        self.assertIn('d["late_90_start_s"]=0', text)
        self.assertIn('d["min_underlying_edge_usd"]=0.0', text)
        self.assertNotIn('d["buy_start_s"]=45', text)
        self.assertNotIn('d["min_underlying_edge_usd"]=25.0', text)
        agents = (root / "AGENTS.md").read_text()
        self.assertIn("last **120s**", agents)
        self.assertIn("edge **$0**", agents)
        ttd = (root / "TECHNICAL_DESIGN.md").read_text()
        self.assertIn("last **120s**", ttd)
        self.assertIn("`min_underlying_edge_usd` is **$0**", ttd)
